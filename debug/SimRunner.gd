...
func _run() -> void:
	...
	var profiles: Array[String] = _ALL_PROFILES if args.all_profiles \
		else [args.profile]
...
func _print_result(profile: String, s: Dictionary) -> void:
	...
	print("  %-18s  Win %5.1f%%  Loss %5.1f%%  Draw %4.1f%%  AvgTurns %4.1f  AvgHP %+.0f" % [
		profile, win_pct, loss_pct, draw_pct, s.avg_turns, avg_hp])
