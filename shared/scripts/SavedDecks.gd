## SavedDecks.gd
## Persists player-created custom decks to user://saved_decks.json.
## All methods are static; no instance needed.
class_name SavedDecks
extends RefCounted

const SAVE_PATH := "user://saved_decks.json"

## Returns all saved decks as { deck_name: Array[card_id] }.
...
## Returns the card list for a saved deck, or [] if not found.
static func get_deck(deck_name: String) -> Array:
	var all := load_all()
	if deck_name in all:
		return all[deck_name] as Array
	return []
