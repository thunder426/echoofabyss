...
##
## Layers:
##   1. Screen distortion shockwave (sonic_wave shader — fake parallax)
##   2. Core flash (additive glow — the ignition point)
##   3. Perspective shockwave ring (Y-squashed procedural donut — implies sphere)
##   4. Volumetric body (3 staggered glow sprites — back/mid/front depth)
##   5. Two-tier particles (hot ejecta + slow smoke wisps)
##   6. Ground shadow (dark ellipse — anchors volume to surface)
##
## Usage:
##   var vfx := VoidBoltImpactVFX.create(impact_global_pos)
...
# ── Colors ───────────────────────────────────────────────────────────────────
const COLOR_CORE_FLASH: Color   = Color(1.0, 0.9, 1.0, 1.0)      # White-hot
const COLOR_VOID_BRIGHT: Color  = Color(0.85, 0.55, 1.0, 0.9)    # Front layer
const COLOR_VOID_MID: Color     = Color(0.6, 0.3, 0.9, 0.7)      # Mid layer
const COLOR_VOID_DARK: Color    = Color(0.35, 0.12, 0.55, 0.5)   # Back layer
const COLOR_RING: Color         = Color(0.7, 0.4, 1.0, 0.8)      # Shockwave ring
const WAVE_TINT: Color          = Color(0.35, 0.10, 0.70, 0.35)  # Dark violet shader tint
...
# ── Timing ───────────────────────────────────────────────────────────────────
const DEBUG_TIME_SCALE: float = 2.5   # Set to 1.0 for normal speed
const TOTAL_DURATION: float = 0.45 * DEBUG_TIME_SCALE
...
func _spawn_distortion(scene: Node) -> void:
	...
	var duration: float = 0.20 * DEBUG_TIME_SCALE
	# Tween on self so it survives rect cleanup
...
func _spawn_core_flash() -> void:
	...
	var tw := create_tween().set_parallel(true)
	tw.tween_property(flash, "scale", flash.scale * 1.6, 0.06 * DEBUG_TIME_SCALE) \
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tw.tween_property(flash, "modulate:a", 0.0, 0.12 * DEBUG_TIME_SCALE) \
		.set_delay(0.04 * DEBUG_TIME_SCALE).set_trans(Tween.TRANS_SINE)
	tw.chain().tween_callback(flash.queue_free)


# ═════════════════════════════════════════════════════════════════════════════
# Layer 3: Perspective Shockwave Ring (Y-squashed → looks 3D)
# ═════════════════════════════════════════════════════════════════════════════
...
func _spawn_ring() -> void:
	...
	tw.tween_property(ring, "scale",
		Vector2(target_scale_x, target_scale_y), 0.22 * DEBUG_TIME_SCALE) \
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT).set_delay(0.03 * DEBUG_TIME_SCALE)
	tw.tween_property(ring, "modulate:a", 0.0, 0.14 * DEBUG_TIME_SCALE) \
		.set_delay(0.12 * DEBUG_TIME_SCALE).set_trans(Tween.TRANS_SINE)
...
func _spawn_body_layers() -> void:
	# Back / Mid / Front — staggered timing creates implied depth
	var configs: Array = [
		{"delay": 0.00 * DEBUG_TIME_SCALE, "color": COLOR_VOID_DARK,   "size_mult": 0.85,
		 "growth": 1.4, "offset": Vector2(randf_range(-6, 6), randf_range(2, 6)),
		 "fade_time": 0.30 * DEBUG_TIME_SCALE, "rotation": randf_range(-0.3, 0.3)},
		{"delay": 0.02 * DEBUG_TIME_SCALE, "color": COLOR_VOID_MID,    "size_mult": 1.0,
		 "growth": 1.5, "offset": Vector2(randf_range(-4, 4), randf_range(-2, 2)),
		 "fade_time": 0.25 * DEBUG_TIME_SCALE, "rotation": randf_range(-0.2, 0.2)},
		{"delay": 0.04 * DEBUG_TIME_SCALE, "color": COLOR_VOID_BRIGHT,  "size_mult": 1.1,
		 "growth": 1.6, "offset": Vector2(randf_range(-5, 5), randf_range(-6, -2)),
		 "fade_time": 0.20 * DEBUG_TIME_SCALE, "rotation": randf_range(-0.25, 0.25)},
	]
	...
	for cfg in configs:
		...
		var tw := create_tween().set_parallel(true)
		tw.tween_property(sprite, "modulate:a", color.a, 0.04 * DEBUG_TIME_SCALE).set_delay(delay)
...
func _spawn_ejecta() -> void:
	...
	burst.emitting = true
	burst.amount = 16
	burst.lifetime = 0.22 * DEBUG_TIME_SCALE
	burst.one_shot = true
	...
	burst.spread = 180.0
	burst.initial_velocity_min = 100.0 / DEBUG_TIME_SCALE
	burst.initial_velocity_max = 220.0 / DEBUG_TIME_SCALE
	burst.damping_min = 150.0 / DEBUG_TIME_SCALE
	burst.damping_max = 300.0 / DEBUG_TIME_SCALE
...
func _spawn_smoke_wisps() -> void:
	...
	wisps.emitting = true
	wisps.amount = 8
	wisps.lifetime = 0.40 * DEBUG_TIME_SCALE
	wisps.one_shot = true
	...
	wisps.spread = 180.0
	wisps.initial_velocity_min = 20.0 / DEBUG_TIME_SCALE
	wisps.initial_velocity_max = 55.0 / DEBUG_TIME_SCALE
	wisps.gravity = Vector2(0, -30.0 / DEBUG_TIME_SCALE)
	wisps.damping_min = 20.0 / DEBUG_TIME_SCALE
	wisps.damping_max = 40.0 / DEBUG_TIME_SCALE
...
func _spawn_ground_shadow() -> void:
	...
	shadow.position = Vector2(0, 8)  # Slightly below center
	...
	var tw := create_tween().set_parallel(true)
	tw.tween_property(shadow, "modulate:a", 0.25, 0.04 * DEBUG_TIME_SCALE)
	tw.tween_property(shadow, "scale", target_scale, 0.12 * DEBUG_TIME_SCALE) \
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tw.chain().tween_property(shadow, "modulate:a", 0.0, 0.25 * DEBUG_TIME_SCALE) \
...
## 64×64 donut ring — soft inner/outer edges, used for the perspective shockwave.
...
## 32×32 soft radial circle — small texture for particle systems.
