...
# TriggerManager Callables registered per rune aura, keyed by rune card id.
# Used to unregister aura handlers when a rune is removed.
var _rune_aura_handlers: Dictionary = {}  # rune_id (String) → Array[Callable]
...
func _setup_enemy_ai() -> void:
	...
	# Configure card pool from current encounter
	if GameManager.current_enemy != null and not GameManager.current_enemy.card_pool.is_empty():
		enemy_ai.card_pool_override = GameManager.current_enemy.card_pool
...
func _try_play_environment(env: EnvironmentCardData) -> void:
	...
	# Tear down previous environment's ritual handlers before replacing
...
func _resolve_on_play_effect(effect_id: String, source: MinionInstance, owner: String = "player") -> void:
	match effect_id:
		...
		"draw_1_card":
			...
			# Enemy draw is handled by EnemyAI._draw_cards() — no-op here
			...
			if owner == "player":
				for m in player_board:
					if _is_void_imp_type(m):
						BuffSystem.apply(m, Enums.BuffType.GRANT_TAUNT, 1, "imp_overseer_aura")
						_refresh_slot_for(m)
				_log("  Imp Overseer: all Void Imps now have Taunt.", _LogType.PLAYER)
...
func _resolve_on_death_effect(minion: MinionInstance) -> void:
	match minion.card_data.on_death_effect:
		"remove_taunt_from_void_imps":
			# Only strip Taunt if no other Imp Overseer remains on the board
			if not _has_imp_overseer_on_board():
				for m in player_board:
					if _is_void_imp_type(m):
...
func _apply_environment_passive(effect_id: String) -> void:
	match effect_id:
		"abyss_rift_passive":
			for minion in player_board:
				if minion.card_data.minion_type == Enums.MinionType.DEMON:
					BuffSystem.apply(minion, Enums.BuffType.TEMP_ATK, 100, "abyss_rift", true)
		...
		"void_rain_passive":
			# Deal 100 Void Bolt damage to BOTH heroes (fires each turn start, player and enemy).
			_log("  Void Rain: 100 Void Bolt damage to both heroes.", _LogType.PLAYER)
			_deal_void_bolt_damage(100)
			_on_hero_damaged("player", 100)
...
## True if at least one Imp Overseer is currently on the player board.
func _has_imp_overseer_on_board() -> bool:
	for m in player_board:
...
func _apply_rune_aura(rune: TrapCardData) -> void:
	...
	match rune.aura_effect_id:
		...
		"blood_rune_aura":
			var heal := 100 * mult
			var h := func(_ctx: EventContext):
				player_hp = mini(player_hp + heal, GameManager.player_hp_max)
				if player_hp_label:
					player_hp_label.text = "HP: %d" % player_hp
	...
	if not entries.is_empty():
		_rune_aura_handlers[rune.id] = entries

## Unregister aura handlers when a rune is removed (destroyed or consumed by ritual).
## No match block needed — each entry stores its own event type.
func _remove_rune_aura(rune: TrapCardData) -> void:
	if _rune_aura_handlers.has(rune.id):
		for entry in _rune_aura_handlers[rune.id]:
			trigger_manager.unregister(entry.event, entry.handler)
		_rune_aura_handlers.erase(rune.id)
...
func _resolve_ritual_effect(effect_id: String) -> void:
	match effect_id:
		...
		"forbidden_insight":
			...
			turn_manager.draw_card()
			turn_manager.draw_card()
			turn_manager.mana = mini(turn_manager.mana + 1, turn_manager.mana_max)
		...
		"soul_cataclysm":
			...
			player_hp = mini(player_hp + 400, GameManager.player_hp_max)
			if player_hp_label:
				player_hp_label.text = "HP: %d" % player_hp
		"abyssal_dominion":
...
func _apply_board_passive_on_summon(passive_id: String, passive_owner: MinionInstance, summoned: MinionInstance) -> void:
	match passive_id:
		...
		"void_imp_taunt_aura":
			if _is_void_imp_type(summoned):
				BuffSystem.apply(summoned, Enums.BuffType.GRANT_TAUNT, 1, "imp_overseer_aura")
